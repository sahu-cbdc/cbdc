...
test("imgbb: key/endpoint/limit ONLY in server/config/imgbb.ts; src/config/imgbb.ts stays public-only", () => {
  ...
  const imgbbLib = read("src/lib/imgbb.ts");
  assert.doesNotMatch(imgbbLib, /IMGBB_API_KEY/, "imgbb.ts must not touch the key");
  assert.match(imgbbLib, /IMGBB_PUBLIC_CONFIG/, "imgbb.ts uses the public compression config");
  for (const f of ["Home", "Admin", "Moderator", "Doner"]) {
    const page = read(`src/pages/${f}.tsx`);
