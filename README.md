...
=======
> `POST /api/admin` (op: `"delete"`) অনুরোধ পাঠায়। এই endpoint-টি **Cloudflare Worker**
...
=======
>   duplicate না বানিয়ে `POST /api/auth` (op: `"resolve-legacy"`, server-secure,
>>>>>>> 69f665ef3c08d211cb53736a98d026fb8416fdf2
...
<<<<<<< HEAD
- client-এ শুধু Firebase-এর publicly-safe web config (`src/config/firebase.ts`;
  Rules-ই আসল নিরাপত্তা) — কোনো `VITE_*` secret env ব্যবহার হয় না
- ImgBB API key শুধু **server-side** `server/config/imgbb.ts`-এ (env `IMGBB_API_KEY`
  দিয়ে override হয়); client bundle/panel-এ কখনো থাকে না — ব্রাউজার সরাসরি ImgBB
  ডাকে না, সব ছবি `POST /api/media` গেটওয়ে দিয়ে যায়
=======
