...
/* ডোনার কার্ড — QR + vCard + PNG download (shared engine; Main Website-ও একই ব্যবহার করে) */
import { qrSVG, downloadDonorCardPng } from "../lib/donorCard";
...
function initPage() {
  ...
  function cardSubject(){
    ...
    return {
      ...
      d:{donorId:x.donorId,bloodGroup:x.group,lastDonation:x.lastDonation||"",
         available:x.available!==false,cardTheme:"green",whatsapp:!!x.phone,
  ...
  function profileCardDL(){
    ...
    CARD_FOR={name:v.name,gender:v.raw.gender,area:v.area,phone:v.phone,
      ...
      age:v.age,lastDonation:v.last,available:v.avail};
  ...
  /* ডোনার কার্ডের shared subject — shared download engine (src/lib/donorCard.ts)
     ব্যবহার করে। Main Website-এর profile-ও একই engine ব্যবহার করে, তাই কার্ড
     দুই জায়গায় হুবহু একই ডিজাইন + QR + dynamic তথ্য নিয়ে নামে। */
  function cardSubjectToShared(){
  ...
  async function dlCard(kind){
    ...
    await downloadDonorCardPng({
      subject: cardSubjectToShared(),
      status: cardStat(),
      kind,
      baseName: base,
      onToast: (msg,k)=>{ if(k==="er")toast(msg,"er"); else toast(msg,k==="ok"?"ok":""); },
    });
    if(SB.mine)logAct("ডোনার কার্ড ডাউনলোড","কার্ড","card");
  }
