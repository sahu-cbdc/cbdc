...
function initPage() {
  ...
  SUBP.security=el=>{
    el.innerHTML=`
      ...
      <div class="note w">${SI.warn(17)}<span>পাসওয়ার্ড কারও সাথে ভাগ করবেন না।
        ${tp("আপনার ভূমিকা ডেটাবেজ থেকে নির্ধারিত হয়।")}</span></div>`;
    bindMe(el,"security");
  };
  ...
  async function deleteOneEntity(d,scope){
    ...
    if(deletingEntities.has(key))return {ok:false,skipped:true};   
  ...
  async function removeDonorsFromList(ids){
    if(bulkDeleteBusy)return false;                 
  ...
  async function roleManageSheet(uid){
    ...
    s.q("#acok")?.addEventListener("click",async()=>{
      ...
      if(savingRoles.has(saveKey))return;                    
  ...
  function dbRender(){
    ...
    if(bc){try{bc.innerHTML=dbCrumbHtml();}catch(e){}}
  }
  ...
  SUBP.gallery=el=>{
    ...
    +`<div class="sec-t">ImgBB সংযোগ</div>
      <div class="card"><div class="kv">
        <div><span>অবস্থা</span><b id="gKeyState">${DB.integr.imgbbKey?"সক্রিয়":"যাচাই হচ্ছে…"}</b></div></div>
  ...
  SUBP.stats=el=>{
    if(!statsReady()){
      el.innerHTML=ptitle("পরিসংখ্যান",tp("তথ্য লোড হচ্ছে…"))
        +skelStats()
