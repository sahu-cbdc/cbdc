...
const toAdminDonor = (d: any) => ({
  ...
  lastDonationDate: d.lastDonationDate || d.last || "",
  available: d.available !== false,
  verified: d.verified !== false,
  suspended: !!d.suspended,
  joined: d.joined || "",
  donations: Number(d.donations ?? d.totalDonations) || 0,
  totalDonations: Number(d.donations ?? d.totalDonations) || 0,
  whatsapp: d.whatsapp || "",
  /* প্রোফাইল ছবি (ImgBB link) — admin round-trip-এ কখনো বাদ পড়ে না */
  photo: d.photo || d.photoURL || "",
  photoURL: d.photo || d.photoURL || "",
  ownerUid: d.ownerUid || d.uid || d.id || "",
});
...
const fromAdminDonor = (d: any) => ({
  ...
  area: d.area || "",
  lastDonationDate: d.last || d.lastDonationDate || "",
  last: d.last || d.lastDonationDate || "",
...
const toDonerDonor = (d: any) => ({
  ...
  occupation: d.occupation || "",
  phone: d.phone || "",
  whatsapp: !!(d.whatsapp && String(d.whatsapp).trim()),
  whatsappNo: typeof d.whatsapp === "string" ? d.whatsapp : (d.whatsappNo || d.phone || ""),
  lastDonation: d.lastDonationDate || d.lastDonation || d.last || "",
  ...
  suspended: !!d.suspended,
  ownerUid: d.ownerUid || d.uid || d.id || "",
  privacy: { showArea: true, showGroup: true, showWhatsapp: !!(d.whatsapp && String(d.whatsapp).trim()) },
});
...
const fromDonerDonor = (d: any) => ({
  ...
  area: d.area || "",
  lastDonationDate: d.lastDonation || d.lastDonationDate || d.last || "",
  last: d.lastDonation || d.lastDonationDate || d.last || "",
